...
interface CityNews {
    id: number;
    title: string;
    category: "ЖКХ" | "Транспорт" | "Медицина" | "Образование";
    description: string;
    fullText: string;
    image: string;
    date: string;
...
interface Business {
    id: number;
    name: string;
    category: "auto" | "food" | "beauty" | "technology";
    description: string;
    rating: number;
    phone?: string;
    address?: string;
...
// === ДАННЫЕ НОВОСТЕЙ (ОПТИМИЗИРОВАННЫЕ) ===
const newsData: CityNews[] = [
    {
        id: 1,
...
const businessesData: Business[] = [
    ...
    { id: 6, name: "Дива", category: "beauty", description: "Все виды стрижек с использванием новейших технологий", rating: 5.0, phone: "89578234567", address: "ул.Железнодорожная, д.60а" },
...
class DataManager {
    ...
    static getTopNews(limit: number = 3): CityNews[] {
        return [...newsData]
...
// === ФУНКЦИИ РЕНДЕРИНГА (ОПТИМИЗИРОВАННЫЕ) ===
function renderNews(newsToRender: CityNews[]): void {
...
function renderBusiness(category: string = "all"): void {
    const grid = document.getElementById("business-grid");
    if (!grid) return;

    const filtered = category === "all" 
        ? businessesData 
        : businessesData.filter(b => b.category === category);
...
class AdvancedSearchEngine {
    private database: SearchResult[] = [];

    constructor() {
        this.buildComprehensiveDatabase();
    }

    private buildComprehensiveDatabase(): void {
        this.database = []; // Очищаем базу перед перестройкой
        ...
        // 6️ДОПОЛНИТЕЛЬНЫЙ КОНТЕНТ (СТАТИЧЕСКИЕ СТРАНИЦЫ)

        // Content from o-gorode.html
        this.database.push({
        ...
        // Content from obshestvenost.html
        this.database.push({
    ...
    search(query: string, filters?: { type?: string; page?: string }): SearchResult[] {
        ...
        // Сортируем по релевантности (заголовок имеет больший вес)
        results.sort((a, b) => {
            ...
            if (!aTitleMatch && bTitleMatch) return 1;
            return 0; // Сохраняем исходный порядок для равных по релевантности
        });
    ...
    // ОБНОВИТЬ БАЗУ (при добавлении новых обращений)
...
// === УЛУЧШЕННАЯ ФУНКЦИЯ ГЛОБАЛЬНОГО ПОИСКА ===
function advancedGlobalSearch(query?: string): void {
...
function showAdvancedSearchResults(term: string, results: SearchResult[]): void {
    ...
    results.forEach((item, index) => {
        // Подсвечиваем совпадения в заголовке и тексте
        ...
        const snippetText = item.text.toLowerCase().includes(term) 
                            ? item.text.substring(Math.max(0, item.text.toLowerCase().indexOf(term) - 30), Math.min(item.text.length, item.text.toLowerCase().indexOf(term) + 120))
    ...
    document.querySelectorAll('.search-result').forEach(el => {
        el.addEventListener('click', () => {
            ...
            if (page) {
                // Remove the modal before navigating
...
function setupBusinessFilters(): void {
    document.querySelectorAll(".f-btn").forEach((btn) => {
        btn.addEventListener("click", (e) => {
            document.querySelectorAll(".f-btn").forEach((b) => b.classList.remove("active"));
            (e.target as HTMLElement).classList.add("active");
            const cat = (e.target as HTMLElement).dataset.cat;
            if (cat) renderBusiness(cat);
        });
    });
}

function setupTicketForm(): void {
    const btnAdd = document.getElementById("btn-add-ticket");
    const titleInput = document.getElementById("in-title") as HTMLInputElement;
    const catSelect = document.getElementById("in-category") as HTMLSelectElement;
    const descTextarea = document.getElementById("in-desc") as HTMLTextAreaElement;
    ...
    btnAdd?.addEventListener("click", () => {
        ...
        renderTickets();
        advancedSearchEngine.rebuildDatabase(); // Rebuild search DB after new ticket
...
// === КАЛЬКУЛЯТОР НАЛОГОВ ===
function setupTaxCalculator(): void {
...
function votePoll(optionIndex: number): void {
    pollData[0].votes[optionIndex]++;
    localStorage.setItem('poll_votes', JSON.stringify(pollData[0].votes));
    renderPoll();
    alert('Спасибо за ваш голос!');
    advancedSearchEngine.rebuildDatabase(); // Rebuild search DB after poll vote
}
...
function setupFeedbackWidget(): void {
    ...
    if (openBtn) openBtn.onclick = openFeedbackModal;
    if (closeBtn) closeBtn.onclick = closeFeedbackModal;
    if (skipBtn) skipBtn.onclick = closeFeedbackModal;
    
    if (submitBtn) {
        submitBtn.onclick = () => {
            const rating = (document.getElementById('feedback-rating-modal') as HTMLSelectElement)?.value;
            const comment = (document.getElementById('feedback-comment-modal') as HTMLTextAreaElement)?.value || '';
            ...
            const feedbackData: FeedbackData[] = JSON.parse(localStorage.getItem('feedback_data') || '[]');
            feedbackData.push({
            ...
            const ratingSelect = document.getElementById('feedback-rating-modal') as HTMLSelectElement;
            const commentText = document.getElementById('feedback-comment-modal') as HTMLTextAreaElement;
...
// === ВСПЛЫВАЮЩЕЕ ОКНО ОТЗЫВА ПРИ ЗАГРУЗКЕ ===
function setupInitialFeedbackPopup(): void {
...
function updateFeedbackStats(): void {
    const feedbackData: FeedbackData[] = JSON.parse(localStorage.getItem('feedback_data') || '[]');
...
// === ЛОКАЛЬНОЕ ХРАНИЛИЩЕ ===
function loadTicketsFromStorage(): void {
...
function saveTicketsToStorage(): void {
    localStorage.setItem("chapaevsk_tickets", JSON.stringify(ticketsData));
}

function setupRandomPopup(): void {
    // Заглушка для функции
    // console.log('Random popup setup');
}
...
// === НАСТРОЙКА ПОИСКА ===
function setupAdvancedSearch(): void {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    advancedSearchEngine.rebuildDatabase(); // Обновляем БД при загрузке с учетом всех страниц
    ...
    setupAdvancedSearch(); // Используем новую функцию
    setupBackToTop();
    renderPoll();
    setupTaxCalculator();
    setupFeedbackWidget();
...
// Экспортируем для использования в других модулях (и в news.html)
(window as any).DataManager = DataManager;
(window as any).advancedSearchEngine = advancedSearchEngine;
(window as any).advancedGlobalSearch = advancedGlobalSearch;
(window as any).newsData = newsData; // Экспортируем newsData, чтобы news.html мог её использовать
(window as any).votePoll = votePoll; // Это нужно, так как функция вызывается из HTML
...
// Прокрутка к форме после перехода с другой страницы
...
// Модальное окно для мобилок (только для o-gorode.html)
...
function openNewsModal(news: any) {
    const modal = document.getElementById('newsModalOverlay');
    if (!modal) return;
    
    const modalImage = document.getElementById('newsModalImage') as HTMLImageElement | null;
    ...
    if (modalCategory) modalCategory.textContent = news.category || 'Новость';
    if (modalTitle) modalTitle.textContent = news.title;
    if (modalText) modalText.textContent = news.fullText || news.description;
...
if (newsModalOverlay) {
    newsModalOverlay.addEventListener('click', function(e: MouseEvent) {
